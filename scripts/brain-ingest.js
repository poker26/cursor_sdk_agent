...
async function ingestJiraProject(workspaceId, projectKey) {
    ...
    for (const issue of issues) {
        await insertBrainEvent({
            ...
            title: `${String(issue.key ?? issue.id ?? "?")}: ${String(issue.summary ?? issue.fields?.summary ?? "")}`,
            payload: issue,
        });
        inserted += 1;
    }
...
async function ingestConfluenceSpace(workspaceId, spaceKey) {
...
async function ingestExchangeInbox(workspaceId) {
...
function extractIssuesFromMcpResult(raw) {
...
function extractPagesFromMcpResult(raw) {
    if (Array.isArray(raw)) {
        return raw;
    }
    if (typeof raw === "object" && raw !== null) {
...
function extractEmailsFromMcpResult(raw) {
    if (Array.isArray(raw)) {
        return raw;
    }
    if (typeof raw === "object" && raw !== null) {
...
async function main() {
    ...
    for (const workspace of targets) {
        ...
        try {
            ...
            console.warn(`Confluence ingest failed: ${error instanceof Error ? error.message : error}`);
        }
        try {
