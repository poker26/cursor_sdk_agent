...
Статус: **запланировано** (после фаз 1–4 persistent brain).
...
### 5c — авто-озвучка и настройки

- Переключатель «Озвучивать ответы» в toolbar (localStorage).
- На `run_finished` при включённом флаге — синтез полного накопленного текста агента.
